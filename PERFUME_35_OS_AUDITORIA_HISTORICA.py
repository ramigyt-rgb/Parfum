...
if page == "◎ Centro Ejecutivo":
    ...
    # ========================================================
    # "Ingresadas" incluye compras, stock inicial y ajustes positivos.
    # No incluye anulaciones/devoluciones de ventas para no contarlas como compras nuevas.
    acquisition_types = ("COMPRA","STOCK_INICIAL","AJUSTE_EDICION","AJUSTE_POSITIVO")
    _acq_sql = ",".join(["?"] * len(acquisition_types))
    acquired_units = safe_float(scalar(
        f"""SELECT COALESCE(SUM(CASE WHEN qty>0 THEN qty ELSE 0 END),0)
            FROM inventory_moves
            WHERE move_type IN ({_acq_sql})""",
        acquisition_types, 0
    ))
    acquired_cost = safe_float(scalar(
    ...
    sold_items_global = safe_float(scalar(
    ...
    sold_moves_global = safe_float(scalar(
    ...
    sold_moves_month = safe_float(scalar(
        """SELECT COALESCE(SUM(-im.qty),0)
           FROM inventory_moves im
           JOIN sales s ON s.id=im.reference_id
           WHERE im.reference_type='SALE'
    ...
    cogs_moves_global = safe_float(scalar(
        """SELECT COALESCE(SUM((-im.qty)*im.unit_cost),0)
           FROM inventory_moves im
           JOIN sales s ON s.id=im.reference_id
           WHERE im.reference_type='SALE'
             AND im.move_type='VENTA'
    ...
    # KPIs ejecutivos: 4 por fila para que se lean completos también en iPad/tablet
    # con el menú lateral abierto. Se usan tarjetas propias en lugar de st.metric
    # para evitar el truncado automático con "..." de Streamlit.
    e1, e2, e3, e4 = st.columns(4)
    with e1:
        pro_kpi("FACTURACIÓN", money(revenue), f"{growth:+.1f}% vs mes anterior", "plum")
    with e2:
        pro_kpi("GANANCIA BRUTA", money(profit), "Resultado bruto del período", "green")
    with e3:
        pro_kpi("MARGEN REAL", pct(margin), f"Meta {TARGET_MARGIN:.0f}%", "green" if margin >= TARGET_MARGIN else "gold")
    with e4:
        pro_kpi("UNIDADES VENDIDAS", f"{units:g}", "Perfumes de 35 ml", "blue")

    e5, e6, e7, e8 = st.columns(4)
    with e5:
        pro_kpi("TICKET PROMEDIO", money(avg_ticket), "Promedio por ticket", "gold")
    with e6:
        pro_kpi("CAJA TEÓRICA", money(cash_balance), "Saldo teórico acumulado", "plum")
    with e7:
        pro_kpi("UNIDADES EN STOCK", f"{stock_units:g}", "Perfumes de 35 ml disponibles", "blue")
    with e8:
        pro_kpi("STOCK A COSTO", money(stock_cost), "Costo del stock actual", "rose")

    e9, e10, e11, e12 = st.columns(4)
    with e9:
        pro_kpi("STOCK A VENTA", money(stock_retail), "Valor si vendés el stock actual", "gold")
    with e10:
        pro_kpi("GANANCIA POTENCIAL", money(stock_potential), "Venta del stock menos su costo", "green")
    with e11:
        pro_kpi("POR COBRAR", money(receivables), "Ventas realizadas pendientes", "rose")
    with e12:
        pro_kpi("POR PAGAR", money(payables), "Obligaciones pendientes", "red")
    ...
    with t3:
        pro_kpi(
            "STOCK + CAJA",
            money(stock_plus_cash),
            "Valor de venta del stock + caja teórica",
            "blue",
        )
    with t4:
        pro_kpi(
    ...
    # Auditoría automática para que compras, ventas y stock cierren entre sí.
    st.markdown("### Conciliación de mercadería")
    st.caption("Acá podés comprobar de dónde sale cada unidad y detectar automáticamente diferencias entre ventas e inventario.")
    ...
    with r1:
        pro_kpi(
            "UNIDADES INGRESADAS",
            f"{acquired_units:g}",
            "Compras + stock inicial + ajustes positivos",
            "blue",
        )
    with r2:
        pro_kpi(
            "COSTO INGRESADO",
            money(acquired_cost),
            "Costo histórico de esas entradas",
            "plum",
        )
    with r3:
        pro_kpi(
            "UNIDADES VENDIDAS",
            f"{sold_items_global:g}",
            "Según el detalle de ventas",
            "green",
        )
    ...
    with r4:
        pro_kpi(
            "SALIDAS POR VENTA",
            f"{sold_moves_global:g}",
            "Según movimientos físicos de stock",
            "gold",
        )
    with r5:
        pro_kpi(
            "COSTO MERCADERÍA VENDIDA",
            money(cogs_moves_global),
            "Costo de las salidas registradas como venta",
            "rose",
        )
    with r6:
        pro_kpi(
            "STOCK ACTUAL",
            f"{stock_units:g}",
            f"Costo actual {money(stock_cost)}",
            "blue",
        )

    _delta_sale_sources = sold_moves_global - sold_items_global
    _outside_month = sold_items_global - units

    # Diagnóstico automático de la típica diferencia 51 vs 52.
    if abs(_delta_sale_sources) > 0.000001:
        st.error(
            f"Hay una diferencia de {abs(_delta_sale_sources):g} unidad(es) entre el detalle de ventas "
            f"({sold_items_global:g}) y las salidas físicas marcadas como VENTA ({sold_moves_global:g}). "
            "Esto significa que al menos una venta y su movimiento de inventario no tienen la misma cantidad."
        )
    elif abs(_outside_month) > 0.000001:
        st.info(
            f"El mes seleccionado muestra {units:g} unidad(es), pero el histórico de ventas completadas tiene "
            f"{sold_items_global:g}. Hay {abs(_outside_month):g} unidad(es) fuera del mes seleccionado."
        )
    elif other_out_units > 0.000001:
        st.warning(
            f"Ventas e inventario coinciden, pero existen {other_out_units:g} unidad(es) de otras salidas "
            "(regalos, pérdidas, ajustes, muestras, etc.). Esas unidades bajan el stock pero no cuentan como venta."
        )
    else:
        st.success(
            f"Conciliación correcta: {acquired_units:g} unidades ingresadas, "
            f"{sold_items_global:g} vendidas y {stock_units:g} en stock."
        )

    with st.expander("Ver auditoría detallada de la diferencia", expanded=abs(_delta_sale_sources) > 0.000001):
        st.write(
            f"**Mes seleccionado:** {units:g} unidades según ventas · "
            f"{sold_moves_month:g} según movimientos de stock."
        )
        st.write(
            f"**Histórico:** {sold_items_global:g} unidades según ventas · "
            f"{sold_moves_global:g} según movimientos de stock."
        )

        if not sales_unit_mismatches.empty:
            st.markdown("**Ventas con cantidades distintas entre venta e inventario**")
            _show_mis = sales_unit_mismatches.copy()
            _show_mis["Producto"] = (
                _show_mis["brand"].fillna("").astype(str).str.strip() + " " +
                _show_mis["name"].fillna("").astype(str).str.strip()
            ).str.strip()
            _show_mis["Venta"] = _show_mis["venta_id"].apply(lambda x: f"#{int(x):06d}" if pd.notna(x) else "")
            _show_mis["Fecha"] = _show_mis["fecha"].astype(str)
            _show_mis["En detalle"] = _show_mis["unidades_detalle"]
            _show_mis["En inventario"] = _show_mis["unidades_inventario"]
            _show_mis["Diferencia"] = _show_mis["diferencia"]
            st.dataframe(
                _show_mis[["Venta","Fecha","sku","Producto","En detalle","En inventario","Diferencia"]],
                use_container_width=True,
                hide_index=True,
            )
            st.caption(
                "La fila que tenga Diferencia = 1 es, normalmente, la unidad que explica un 51 vs 52. "
                "No se corrige automáticamente para no alterar una venta sin que la revises primero."
            )
        else:
            st.success("No hay ventas con diferencias entre sale_items e inventory_moves.")

        if not other_out_df.empty:
            st.markdown("**Otras salidas de stock que NO son ventas**")
            st.dataframe(other_out_df, use_container_width=True, hide_index=True)
        else:
            st.caption("No hay otras salidas negativas de stock fuera de ventas.")

        _all_moves_balance = safe_float(scalar(
            "SELECT COALESCE(SUM(qty),0) FROM inventory_moves", default=0
        ))
        st.caption(
            f"Control final del libro de stock: la suma de todos los movimientos da "
            f"{_all_moves_balance:g} unidad(es), y el inventario visible muestra {stock_units:g}."
        )

    # ========================================================
    # AUDITORÍA HISTÓRICA PROFUNDA
    # ========================================================
    with st.expander("Auditoría histórica · encontrar la unidad faltante y costos incorrectos", expanded=True):
        st.caption(
            "Esta auditoría NO modifica nada. Compara compras, ventas y movimientos de stock para mostrar "
            "exactamente dónde se cargó una cantidad o un costo distinto."
        )

        ac1, ac2, ac3, ac4 = st.columns(4)
        expected_cost = ac1.number_input(
            "Costo unitario esperado",
            min_value=0.0,
            value=1900.0,
            step=100.0,
            key="audit_expected_unit_cost",
            help="Usá 1.900 si todos estos perfumes de 35 ml te costaron lo mismo."
        )
        expected_bought = ac2.number_input(
            "Unidades que recordás haber comprado",
            min_value=0.0,
            value=144.0,
            step=1.0,
            key="audit_expected_bought",
        )
        expected_sold = ac3.number_input(
            "Unidades que recordás haber vendido",
            min_value=0.0,
            value=52.0,
            step=1.0,
            key="audit_expected_sold",
        )
        physical_stock = ac4.number_input(
            "Stock físico que contaste",
            min_value=0.0,
            value=float(stock_units),
            step=1.0,
            key="audit_physical_stock",
        )

        purchase_items_units = safe_float(scalar(
            "SELECT COALESCE(SUM(qty),0) FROM purchase_items", default=0
        ))
        purchase_items_cost = safe_float(scalar(
            "SELECT COALESCE(SUM(qty*landed_unit_cost),0) FROM purchase_items", default=0
        ))
        purchase_move_units = safe_float(scalar(
            """SELECT COALESCE(SUM(qty),0) FROM inventory_moves
               WHERE move_type='COMPRA' AND qty>0""",
            default=0
        ))
        initial_adjust_units = safe_float(scalar(
            """SELECT COALESCE(SUM(qty),0) FROM inventory_moves
               WHERE qty>0 AND move_type<>'COMPRA'""",
            default=0
        ))

        expected_acq_cost = acquired_units * expected_cost
        expected_cogs = sold_moves_global * expected_cost
        acq_cost_gap = expected_acq_cost - acquired_cost
        cogs_cost_gap = expected_cogs - cogs_moves_global

        diff_bought = expected_bought - acquired_units
        diff_sold = expected_sold - sold_items_global
        diff_physical = physical_stock - stock_units

        st.markdown("#### Control contra lo que realmente hiciste")
        cc1, cc2, cc3, cc4 = st.columns(4)
        with cc1:
            pro_kpi(
                "ENTRADAS FALTANTES",
                f"{diff_bought:+g}",
                f"Recordás {expected_bought:g} · sistema {acquired_units:g}",
                "rose" if abs(diff_bought) > 0.000001 else "green",
            )
        with cc2:
            pro_kpi(
                "VENTAS FALTANTES",
                f"{diff_sold:+g}",
                f"Recordás {expected_sold:g} · sistema {sold_items_global:g}",
                "rose" if abs(diff_sold) > 0.000001 else "green",
            )
        with cc3:
            pro_kpi(
                "DIF. STOCK FÍSICO",
                f"{diff_physical:+g}",
                f"Contaste {physical_stock:g} · sistema {stock_units:g}",
                "rose" if abs(diff_physical) > 0.000001 else "green",
            )
        with cc4:
            theoretical_from_memory = expected_bought - expected_sold
            pro_kpi(
                "STOCK SEGÚN TU HISTORIA",
                f"{theoretical_from_memory:g}",
                f"{expected_bought:g} compradas − {expected_sold:g} vendidas",
                "blue",
            )

        if abs(diff_bought) == 1 and abs(diff_sold) == 1 and abs(diff_physical) < 0.000001:
            st.warning(
                "Patrón detectado: falta exactamente 1 entrada y 1 venta en el historial, pero ambas se compensan. "
                "Por eso el stock actual puede estar perfecto aunque el histórico diga 143 / 51 en vez de 144 / 52."
            )

        st.markdown("#### Control de costos a $ {:,.0f} por unidad".format(expected_cost).replace(",", "."))
        kc1, kc2, kc3, kc4 = st.columns(4)
        with kc1:
            pro_kpi(
                "COSTO ESPERADO ENTRADAS",
                money(expected_acq_cost),
                f"{acquired_units:g} × {money(expected_cost)}",
                "blue",
            )
        with kc2:
            pro_kpi(
                "COSTO REGISTRADO ENTRADAS",
                money(acquired_cost),
                f"Diferencia {money(acq_cost_gap)}",
                "rose" if abs(acq_cost_gap) > 0.01 else "green",
            )
        with kc3:
            pro_kpi(
                "COSTO ESPERADO VENTAS",
                money(expected_cogs),
                f"{sold_moves_global:g} × {money(expected_cost)}",
                "blue",
            )
        with kc4:
            pro_kpi(
                "COSTO REGISTRADO VENTAS",
                money(cogs_moves_global),
                f"Diferencia {money(cogs_cost_gap)}",
                "rose" if abs(cogs_cost_gap) > 0.01 else "green",
            )

        if expected_cost > 0 and abs(acq_cost_gap) > 0.01:
            equiv = acq_cost_gap / expected_cost
            st.error(
                f"En las entradas faltan {money(acq_cost_gap)} de costo respecto de {money(expected_cost)} por unidad. "
                f"Eso equivale a {equiv:g} unidad(es) con costo faltante o cargado distinto."
            )
        if expected_cost > 0 and abs(cogs_cost_gap) > 0.01:
            equiv_sale = cogs_cost_gap / expected_cost
            st.error(
                f"En las salidas por venta faltan {money(cogs_cost_gap)} de costo. "
                f"Eso equivale a {equiv_sale:g} unidad(es) a {money(expected_cost)}."
            )

        tab_batches, tab_sales_days, tab_costs, tab_cross, tab_ledger = st.tabs(
            ["Entradas / lotes", "Ventas por día", "Costos sospechosos", "Cruce compras ↔ stock", "Libro completo"]
        )

        with tab_batches:
            batch_df = qdf(
                """SELECT im.date AS Fecha,
                          im.move_type AS Tipo,
                          im.reference_type AS Referencia,
                          im.reference_id AS ID_ref,
                          COALESCE(pu.invoice,'') AS Comprobante,
                          SUM(im.qty) AS Unidades,
                          SUM(im.qty*im.unit_cost) AS Costo_total,
                          CASE WHEN SUM(im.qty)<>0
                               THEN SUM(im.qty*im.unit_cost)/SUM(im.qty)
                               ELSE 0 END AS Costo_unitario_promedio
                   FROM inventory_moves im
                   LEFT JOIN purchases pu
                     ON im.reference_type='PURCHASE' AND pu.id=im.reference_id
                   WHERE im.qty>0
                   GROUP BY im.date,im.move_type,im.reference_type,im.reference_id,pu.invoice
                   ORDER BY im.date,im.id"""
            )
            if batch_df.empty:
                st.info("No hay entradas de stock registradas.")
            else:
                batch_df["Esperado_a_costo_base"] = batch_df["Unidades"] * expected_cost
                batch_df["Diferencia_costo"] = batch_df["Esperado_a_costo_base"] - batch_df["Costo_total"]
                st.dataframe(
                    batch_df,
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        ...
                        "Costo_unitario_promedio": st.column_config.NumberColumn("Costo unitario prom.", format="$ %.0f"),
                        "Esperado_a_costo_base": st.column_config.NumberColumn("Esperado", format="$ %.0f"),
                        "Diferencia_costo": st.column_config.NumberColumn("Dif. costo", format="$ %.0f"),
                    }
                )
                st.caption(
                    "Buscá especialmente una entrada de 43 unidades donde esperabas 44, "
                    "o cualquier lote cuyo costo unitario no sea el esperado."
                )

        with tab_sales_days:
            sales_day_df = qdf(
                """SELECT s.date AS Fecha,
                          COUNT(DISTINCT s.id) AS Tickets,
                          SUM(si.qty) AS Unidades,
                          SUM(si.subtotal) AS Facturacion,
                          SUM(si.qty*si.unit_cost) AS Costo_registrado
                   FROM sales s
            ...
            if sales_day_df.empty:
                st.info("No hay ventas completadas.")
            else:
                sales_day_df["Costo_esperado"] = sales_day_df["Unidades"] * expected_cost
                sales_day_df["Diferencia_costo"] = sales_day_df["Costo_esperado"] - sales_day_df["Costo_registrado"]
                st.dataframe(
                    sales_day_df,
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        ...
                        "Costo_registrado": st.column_config.NumberColumn("Costo registrado", format="$ %.0f"),
                        "Costo_esperado": st.column_config.NumberColumn("Costo esperado", format="$ %.0f"),
                        "Diferencia_costo": st.column_config.NumberColumn("Dif. costo", format="$ %.0f"),
                    }
                )
                st.caption(
                    "Esta vista permite reconstruir tus 38 primeras ventas y las 14 posteriores. "
                    "Si el sistema totaliza 51, acá vas a ver en qué fecha quedó una unidad sin registrar."
                )

        with tab_costs:
            cost_move_df = qdf(
                """SELECT im.id AS Movimiento,
                          im.date AS Fecha,
                          im.move_type AS Tipo,
                          p.sku AS SKU,
                          p.brand || ' ' || p.name AS Producto,
                          im.qty AS Cantidad,
                          im.unit_cost AS Costo_unitario,
                          im.reference_type AS Ref_tipo,
                          im.reference_id AS Ref_id,
                          COALESCE(im.notes,'') AS Notas
                   FROM inventory_moves im
                   JOIN products p ON p.id=im.product_id
                   WHERE ABS(COALESCE(im.unit_cost,0)-?)>0.01
                     AND im.move_type IN ('COMPRA','VENTA','STOCK_INICIAL','AJUSTE_EDICION','AJUSTE_POSITIVO')
                   ORDER BY im.date,im.id""",
                (expected_cost,)
            )
            sale_cost_df = qdf(
                """SELECT s.id AS Venta,
                          ...
                          si.qty AS Cantidad,
                          si.unit_cost AS Costo_unitario,
                          si.unit_price AS Precio_venta
                   FROM sale_items si
                   JOIN sales s ON s.id=si.sale_id
                   JOIN products p ON p.id=si.product_id
                   WHERE s.status='Completada'
                     AND ABS(COALESCE(si.unit_cost,0)-?)>0.01
                   ORDER BY s.date,s.id""",
                (expected_cost,)
            )
            purchase_cost_df = qdf(
                """SELECT pu.id AS Compra,
                          pu.date AS Fecha,
                          COALESCE(pu.invoice,'') AS Comprobante,
                          p.sku AS SKU,
                          p.brand || ' ' || p.name AS Producto,
                          pi.qty AS Cantidad,
                          pi.unit_price AS Precio_compra,
                          pi.landed_unit_cost AS Costo_final
                   FROM purchase_items pi
                   JOIN purchases pu ON pu.id=pi.purchase_id
                   JOIN products p ON p.id=pi.product_id
                   WHERE ABS(COALESCE(pi.landed_unit_cost,0)-?)>0.01
                      OR ABS(COALESCE(pi.unit_price,0)-?)>0.01
                   ORDER BY pu.date,pu.id""",
                (expected_cost, expected_cost)
            )

            if cost_move_df.empty and sale_cost_df.empty and purchase_cost_df.empty:
                st.success(f"No encontré costos distintos de {money(expected_cost)}.")
            else:
                if not cost_move_df.empty:
                    st.markdown("**Movimientos de inventario con costo distinto**")
                    st.dataframe(cost_move_df, use_container_width=True, hide_index=True)
                if not sale_cost_df.empty:
                    st.markdown("**Líneas de venta con costo distinto**")
                    st.dataframe(sale_cost_df, use_container_width=True, hide_index=True)
                if not purchase_cost_df.empty:
                    st.markdown("**Líneas de compra con costo distinto**")
                    st.dataframe(purchase_cost_df, use_container_width=True, hide_index=True)

                st.warning(
                    "No corrijo estos registros automáticamente: primero identificamos cuáles son reales. "
                    "Así evitamos alterar una compra o venta correcta."
                )

        with tab_cross:
            p_items = qdf(
                """SELECT pi.purchase_id AS Compra,
                          pi.product_id,
                          SUM(pi.qty) AS Unidades_compra,
                          SUM(pi.qty*pi.landed_unit_cost) AS Costo_compra
                   FROM purchase_items pi
                   GROUP BY pi.purchase_id,pi.product_id"""
            )
            p_moves = qdf(
                """SELECT im.reference_id AS Compra,
                          im.product_id,
                          SUM(im.qty) AS Unidades_stock,
                          SUM(im.qty*im.unit_cost) AS Costo_stock
                   FROM inventory_moves im
                   WHERE im.reference_type='PURCHASE'
                     AND im.move_type='COMPRA'
                   GROUP BY im.reference_id,im.product_id"""
            )
            if p_items.empty and p_moves.empty:
                st.info("No hay compras para cruzar.")
            else:
                cross = pd.merge(p_items, p_moves, on=["Compra","product_id"], how="outer")
                for c in ["Unidades_compra","Costo_compra","Unidades_stock","Costo_stock"]:
                    cross[c] = pd.to_numeric(cross.get(c, 0), errors="coerce").fillna(0)
                cross["Dif_unidades"] = cross["Unidades_stock"] - cross["Unidades_compra"]
                cross["Dif_costo"] = cross["Costo_stock"] - cross["Costo_compra"]
                products_cross = qdf("SELECT id AS product_id,sku,brand,name FROM products")
                cross = cross.merge(products_cross, on="product_id", how="left")
                cross["Producto"] = (
                    cross["brand"].fillna("").astype(str).str.strip() + " " +
                    cross["name"].fillna("").astype(str).str.strip()
                ).str.strip()
                cross_view = cross[
                    ["Compra","sku","Producto","Unidades_compra","Unidades_stock","Dif_unidades","Costo_compra","Costo_stock","Dif_costo"]
                ].sort_values(["Compra","Producto"])
                st.dataframe(cross_view, use_container_width=True, hide_index=True)

                bad_cross = cross[
                    (cross["Dif_unidades"].abs()>0.000001) |
                    (cross["Dif_costo"].abs()>0.01)
                ]
                if bad_cross.empty:
                    st.success("Cada compra coincide con su movimiento de ingreso al inventario.")
                else:
                    st.error(
                        f"Encontré {len(bad_cross)} línea(s) donde la compra y el movimiento de stock no coinciden."
                    )

            st.markdown("**Resumen de fuentes**")
            source_summary = pd.DataFrame([
                {"Fuente":"purchase_items", "Unidades":purchase_items_units, "Costo":purchase_items_cost},
                {"Fuente":"movimientos COMPRA", "Unidades":purchase_move_units, "Costo":safe_float(scalar("SELECT COALESCE(SUM(qty*unit_cost),0) FROM inventory_moves WHERE move_type='COMPRA' AND qty>0", default=0))},
                {"Fuente":"otras entradas positivas", "Unidades":initial_adjust_units, "Costo":safe_float(scalar("SELECT COALESCE(SUM(qty*unit_cost),0) FROM inventory_moves WHERE qty>0 AND move_type<>'COMPRA'", default=0))},
                {"Fuente":"sale_items completadas", "Unidades":sold_items_global, "Costo":safe_float(scalar("SELECT COALESCE(SUM(si.qty*si.unit_cost),0) FROM sale_items si JOIN sales s ON s.id=si.sale_id WHERE s.status='Completada'", default=0))},
                {"Fuente":"movimientos VENTA", "Unidades":sold_moves_global, "Costo":cogs_moves_global},
                {"Fuente":"stock actual", "Unidades":stock_units, "Costo":stock_cost},
            ])
            st.dataframe(
                source_summary,
                use_container_width=True,
                hide_index=True,
                column_config={"Costo": st.column_config.NumberColumn("Costo", format="$ %.0f")}
            )
        ...
        with tab_ledger:
            ledger_df = qdf(
                """SELECT im.id AS ID,
                          ...
                          im.qty AS Cantidad,
                          im.unit_cost AS Costo_unitario,
                          im.qty*im.unit_cost AS Importe_costo,
                          COALESCE(im.reference_type,'') AS Ref_tipo,
                          COALESCE(im.reference_id,'') AS Ref_id,
                          COALESCE(im.notes,'') AS Notas,
                          COALESCE(im.user_name,'') AS Usuario
                   FROM inventory_moves im
                   JOIN products p ON p.id=im.product_id
                   ORDER BY im.date,im.id"""
            )
            if ledger_df.empty:
                ...
                ledger_df["Costo_OK"] = (
                    pd.to_numeric(ledger_df["Costo_unitario"], errors="coerce").fillna(0)
                    .sub(expected_cost).abs() <= 0.01
                )
                st.dataframe(
                    ledger_df,
                    use_container_width=True,
                    ...
                    height=520,
                    column_config={
                        "Costo_unitario": st.column_config.NumberColumn("Costo unitario", format="$ %.0f"),
                        "Importe_costo": st.column_config.NumberColumn("Importe costo", format="$ %.0f"),
                        "Costo_OK": st.column_config.CheckboxColumn(f"Costo = {money(expected_cost)}"),
                    }
                )
                st.download_button(
                    "Descargar auditoría CSV",
                    data=ledger_df.to_csv(index=False).encode("utf-8-sig"),
                    file_name="auditoria_movimientos_perfumeria.csv",
                    mime="text/csv",
                    use_container_width=True,
                )
    ...
    with c2:
        ...
        inv = stock_df()
        low = inv[(inv.min_stock>0) & (inv.stock <= inv.min_stock) & (inv.active==1)] if not inv.empty else pd.DataFrame()
        no_stock = inv[(inv.stock <= 0) & (inv.active==1) & (inv.sellable==1) & ((inv.sale_price>0) | (inv.min_stock>0))] if not inv.empty else pd.DataFrame()
        if not low.empty:
...
# ============================================================
    ...
    st.markdown('<div class="edit-callout"><b>Importante:</b> Catálogo e Inventario tienen editores especializados y más cómodos. Esta pantalla existe para que también puedas corregir clientes, proveedores, ventas, compras, caja, reservas, promociones y demás tablas sin salir de la app.</div>',unsafe_allow_html=True)
    labels={
        ...
        'Reservas':'reservations','Cuentas corrientes':'account_ledger','Promociones':'promotions','Campañas':'campaign_log','Configuración':'settings','Usuarios':'users','Pools decant':'decant_pools'
    }
