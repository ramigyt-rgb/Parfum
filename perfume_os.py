...
# Un solo archivo Streamlit + SQLite autogenerado
# ============================================================

APP_NAME = "PERFUME OS"
APP_VERSION = "1.0.0"
DB_PATH = Path(__file__).with_name("perfume_os.db")
...
def scalar(sql, params=(), default=0):
    row = execute(sql, params, commit=False).fetchone()
    if not row:
        return default
    val = row[0]
...
def table_columns(table):
    return [r[1] for r in execute(f"PRAGMA table_info({table})", commit=False).fetchall()]
    ...
    with zipfile.ZipFile(bio, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr(
            ...
            (
                ...
                "El archivo perfume_os.db contiene la base completa si desea respaldarlo directamente."
            ).encode("utf-8")
        )
    bio.seek(0)
    return bio.getvalue()
...
# ============================================================
    ...
    with t3:
        ...
        if DB_PATH.exists():
            st.download_button(
                "Descargar base SQLite completa",
                data=DB_PATH.read_bytes(),
                file_name="perfume_os.db",
                mime="application/octet-stream",
                use_container_width=True,
            )
        st.info("La app crea automáticamente perfume_os.db al lado del archivo .py. Para mover la app con todos sus datos, guardá ambos archivos o descargá un backup.")
    ...
    with t4:
        ...
        st.code(f"Base: {DB_PATH}\nVersión: {APP_VERSION}\nPython: sistema local\nFecha: {now_iso()}")
        if st.button("Optimizar base"):
            execute("VACUUM")
            st.success("Base optimizada.")

# -----------------------------
# FOOTER
# -----------------------------
st.divider()
st.caption(f"{APP_NAME} · v{APP_VERSION} · {BUSINESS_NAME} · Datos locales persistentes en SQLite")
