...
def _get_google_credentials_source():
    ...
    try:
        if "gcp_service_account" in st.secrets:
            return "streamlit_secrets", dict(st.secrets["gcp_service_account"])
    except Exception:
        pass

    # 2) Desarrollo local con service_account.json
    if CREDENTIALS_FILE.exists():
        return "local_json", str(CREDENTIALS_FILE)

    raise RuntimeError(
        "No encontré credenciales de Google. En Streamlit Cloud cargá la sección "
        "[gcp_service_account] en Settings > Secrets. En local, podés usar "
        "service_account.json en la misma carpeta del archivo .py."
    )
...
def get_backend():
    ...
    try:
        ...
        if sheet_id:
            spreadsheet = gc.open_by_key(str(sheet_id).strip())
        else:
            # Fallback útil para uso local o si todavía no cargaste el ID.
        ...
        raise RuntimeError(
            ...
            "En Streamlit Cloud verificá GOOGLE_SHEET_ID y [gcp_service_account]. "
            "Además, compartí la planilla como Editor con el client_email "
