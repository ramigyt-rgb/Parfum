...
def _require_google_credentials():
    if not CREDENTIALS_FILE.exists():
        raise RuntimeError(
            "Falta service_account.json. Descargá la clave JSON de una Cuenta de Servicio de Google Cloud "
            "y guardala en la misma carpeta que perfume_os_sheets.py con el nombre service_account.json."
        )
...
def get_backend():
    ...
    try:
        ...
        raise RuntimeError(
            f"No pude abrir el Google Sheet '{SPREADSHEET_NAME}'. "
            "Verificá que exista y que esté compartido como Editor con el client_email de service_account.json. "
            f"Detalle: {e}"
        )
...
try:
    ...
    st.markdown(
        "**Para iniciar:** creá un Google Sheet llamado `Parfum`, creá una Cuenta de Servicio de Google, "
        "compartí `Parfum` como Editor con el `client_email` de esa cuenta y guardá su JSON como "
        "`service_account.json` junto a este archivo `.py`."
    )
    st.stop()
...
with st.sidebar:
    ...
    st.caption(f"☁ Google Sheets · {SPREADSHEET_NAME}")
...
# ============================================================
    ...
    with t4:
        ...
        st.code(f"Google Sheet: {SPREADSHEET_NAME}\nVersión: {APP_VERSION}\nPersistencia: Google Sheets\nFecha: {now_iso()}")
...
# -----------------------------
st.divider()
st.caption(f"{APP_NAME} · v{APP_VERSION} · {BUSINESS_NAME} · Persistencia 100% en Google Sheets: {SPREADSHEET_NAME}")
