...
def _diagnostic_hint_for_exception(exc):
    ...
    if "quota" in low or "429" in low:
        return "Google alcanzó temporalmente la cuota de lecturas. Esta versión evita releer la estructura en cada clic. Esperá cerca de un minuto y reintentá."
...
def render_google_sheets_diagnostic(original_error=None):
    ...
    st.markdown(
        ...
        <div class="hero">
          ...
          <p>PERFUME OS prueba cada capa por separado para identificar el punto exacto de falla. Versión SAFE 2.1.2.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _refresh_worksheet_cache():
    """
    Una única consulta de metadatos obtiene todas las pestañas.
    Se conserva en memoria mientras viva la instancia de Streamlit.
    """
    backend = get_backend()
...
def _worksheet_for_table(table):
    """
    Devuelve la pestaña exacta sin confundir errores 429/403 con 'no existe'.
    """
    backend = get_backend()
    ...
    try:
        ...
        # Si otra instancia la creó entre medio, refrescamos y la reutilizamos.
        if "already exists" in str(e).lower():
            cache = _refresh_worksheet_cache()
            if title in cache:
                return cache[title]
        # Un 429, 403, etc. se propaga al detector; NO intenta crear otra hoja.
        raise
...
def ensure_sheet_structure():
    """
    Solo comprueba la estructura una vez por instancia.
    Hace 1 lectura de metadatos, no una lectura completa por pestaña.
    Las hojas nuevas reciben su encabezado; las existentes se respetan.
    """
    backend = get_backend()
    with backend["lock"]:
...
def init_db():
    ...
    # Streamlit vuelve a ejecutar el script con cada clic.
    # Si ya cargamos Google Sheets en esta instancia, no hacemos ninguna llamada extra.
