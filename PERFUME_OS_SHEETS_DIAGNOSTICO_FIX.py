...
APP_NAME = "PERFUME OS"
APP_VERSION = "2.1.1-SHEETS-DIAG-FIX"
...
def _diagnostic_credentials_candidate():
    ...
    for json_secret_name in [
        ...
        try:
            if json_secret_name in st.secrets:
                raw = str(st.secrets[json_secret_name]).strip()
                if not raw:
                    return None, json_secret_name, "El Secret existe pero está vacío."
                try:
                    candidate = json.loads(raw)
                except json.JSONDecodeError as e:
                    return None, json_secret_name, (
                        f"JSON inválido: línea {e.lineno}, columna {e.colno}: {e.msg}"
                    )
                return _normalize_private_key(candidate), json_secret_name, None
        except Exception as e:
            return None, json_secret_name, f"No pude leer el Secret: {type(e).__name__}: {e}"
...
def _diagnostic_hint_for_exception(exc):
    ...
    if "quota" in low or "429" in low:
        return "Google está limitando temporalmente las llamadas por cuota. Esperá y reintentá."
...
def run_google_sheets_diagnostic(deep_write_test=True):
    ...
    if cred_error:
        add(
            ...
            "Recomendado: pegá el JSON COMPLETO en GOOGLE_SERVICE_ACCOUNT_JSON."
        )
        return results
...
def render_google_sheets_diagnostic(original_error=None):
    ...
    minimal_config = (
        ...
        'GOOGLE_SERVICE_ACCOUNT_JSON = "PEGAR_EL_JSON_COMPLETO_DE_GOOGLE_CLOUD_COMO_UN_SOLO_SECRET"\n'
    )
...
def _get_google_credentials_source():
            ...
            if json_secret_name in st.secrets:
                ...
                if raw:
                    ...
                    if _looks_like_service_account(candidate):
                        return f"streamlit_json:{json_secret_name}", candidate
                    raise RuntimeError(
                        f"{json_secret_name} existe, pero no parece ser un JSON válido "
                        "de una Cuenta de Servicio de Google."
                    )
        except json.JSONDecodeError as e:
            raise RuntimeError(
                f"{json_secret_name} está cargado, pero el JSON quedó mal pegado o incompleto. "
                f"Detalle: {e}"
            )
    ...
    raise RuntimeError(
        ...
        "En Streamlit Cloud cargá GOOGLE_SERVICE_ACCOUNT_JSON (o GCP_SERVICE_ACCOUNT_JSON) con el JSON COMPLETO "
