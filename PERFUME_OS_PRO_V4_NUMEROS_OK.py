...
st.markdown(
    ...
    [data-testid="stMetricValue"]{
        ...
        font-size:clamp(1.15rem,1.6vw,1.9rem)!important;
    ...
    .pro-card .value{
        font-size:clamp(1.15rem,1.55vw,1.55rem);font-weight:850;letter-spacing:-.035em;
        color:#211b22;margin:.15rem 0;white-space:nowrap;overflow:visible;text-overflow:clip;
...
if page == "◎ Centro Ejecutivo":
    ...
    # KPIs ejecutivos en tarjetas HTML propias.
    # Evitamos st.metric en grillas estrechas porque Streamlit aplica
    # text-overflow: ellipsis y puede mostrar importes como "$ 112....".
    # Estas tarjetas siempre muestran el número completo.
    cols = st.columns(6)
    with cols[0]:
        pro_kpi("FACTURACIÓN", money(revenue), f"{growth:+.1f}% vs mes anterior", "gold")
    with cols[1]:
        pro_kpi("GANANCIA BRUTA", money(profit), "Resultado bruto del período", "green")
    with cols[2]:
        pro_kpi("MARGEN REAL", pct(margin), f"Meta {TARGET_MARGIN:.0f}%", "green" if margin >= TARGET_MARGIN else "rose")
    with cols[3]:
        pro_kpi("UNIDADES", f"{units:g}", "Unidades vendidas", "plum")
    with cols[4]:
        pro_kpi("TICKET PROMEDIO", money(avg_ticket), f"{tickets} ticket(s)", "blue")
    with cols[5]:
        pro_kpi("CAJA TEÓRICA", money(cash_balance), "Saldo acumulado", "plum")

    cols2 = st.columns(5)
    with cols2[0]:
        pro_kpi("STOCK A COSTO", money(stock_cost), "Capital en inventario", "gold")
    with cols2[1]:
        pro_kpi("STOCK A VENTA", money(stock_retail), "Valor potencial de venta", "plum")
    with cols2[2]:
        pro_kpi("GANANCIA POTENCIAL", money(stock_potential), "Margen del stock actual", "green")
    with cols2[3]:
        pro_kpi("POR COBRAR", money(receivables), "Saldo de clientes", "rose" if receivables > 0 else "green")
    with cols2[4]:
        pro_kpi("POR PAGAR", money(payables), "Saldo con proveedores", "rose" if payables > 0 else "green")
...
# ============================================================
    ...
    if not inv.empty:
        ...
        if choices.empty:
            ...
            st.markdown(
                f'<div class="insight"><b>{prow.brand} {prow["name"]}</b> · stock {prow.stock:g} · '
                f'costo {money(prow.avg_cost)} · precio {money(price)} · margen <b>{_p_margin:.1f}%</b>'
                f'{" · ⚠ debajo de meta" if _p_margin < TARGET_MARGIN else ""}</div>',
                unsafe_allow_html=True
            )
            if add:
