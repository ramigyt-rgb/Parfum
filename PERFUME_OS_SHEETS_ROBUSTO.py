...
def _get_google_credentials_source():
    """
    Acepta varias estructuras de Streamlit Secrets para evitar depender
    de un único nombre de sección.
    """
    ...
    # 2) JSON completo guardado como string
    json_secret_names = [
        ...
        for key in json_secret_names:
            if key in st.secrets:
                ...
                if isinstance(raw, str) and raw.strip():
                    ...
                    candidate = _normalize_private_key(candidate)
                    if _looks_like_service_account(candidate):
                        return f"streamlit_json:{key}", candidate
    except Exception:
        pass

    # 3) Campos planos en la raíz de Secrets
    try:
        flat = {}
        for key in [
            ...
            if key in st.secrets:
                flat[key] = st.secrets[key]
        flat = _normalize_private_key(flat)
        if _looks_like_service_account(flat):
            return "streamlit_flat", flat
    except Exception:
        pass

    # 4) Local: archivo JSON
    if CREDENTIALS_FILE.exists():
        return "local_json", str(CREDENTIALS_FILE)

    visible_keys = _safe_secret_keys()
    visible = ", ".join(visible_keys) if visible_keys else "(ninguno)"
    raise RuntimeError(
        "No encontré credenciales válidas de Google en Streamlit Secrets. "
        f"Secrets detectados por la app: {visible}. "
        "Acepto [gcp_service_account], [google_service_account], [service_account], "
        "[google], [gcp], un JSON completo en GCP_SERVICE_ACCOUNT_JSON, "
        "o los campos de la cuenta de servicio en la raíz. "
        "No se muestran valores sensibles."
    )
...
def _worksheet_for_table(table):
    ...
    try:
        ...
        return ss.add_worksheet(title=title, rows=1000, cols=max(20, len(cols) + 3))
    ...
    with backend["lock"]:
        for table in TABLE_ORDER:
            ...
            if not values:
                ...
                # Preserve existing matching data, but normalize the header/order safely.
                old_header = values[0]
                old_rows = values[1:]
                normalized = []
                for row in old_rows:
                    d = {old_header[i]: row[i] if i < len(row) else "" for i in range(len(old_header))}
                    normalized.append([d.get(c, "") for c in cols])
                _gspread_update(ws, [cols] + normalized)
...
def reload_from_sheets():
    ...
    with backend["lock"]:
        ...
        # Seed only if the corresponding Google Sheet is empty.
        defaults = {
...
try:
    ...
    st.error("No pude conectar PERFUME OS con Google Sheets.")
    st.code(str(_google_error))
    st.markdown(
        "**Para iniciar en Streamlit Cloud:** cargá `GOOGLE_SHEET_ID` y la sección "
        "`[gcp_service_account]` en **Settings → Secrets**. Después compartí el Google Sheet "
        "como **Editor** con el `client_email` de esa Cuenta de Servicio. "
        "No necesitás subir `service_account.json` a GitHub."
    )
    st.stop()
...
st.caption(f"{APP_NAME} · v{APP_VERSION} · {BUSINESS_NAME} · Persistencia 100% en Google Sheets")
