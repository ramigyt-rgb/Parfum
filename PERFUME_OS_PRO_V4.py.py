...
_HISTORY_RECON_DEFAULTS = {
    ...
    "history_recon_cutoff": "2026-09-14",
...
def history_reconciliation():
    ...
    try:
        ...
        start = date(2026, 9, 1)
    try:
        cutoff = date.fromisoformat(get_setting("history_recon_cutoff", "2026-09-14"))
    except Exception:
...
if page == "◎ Centro Ejecutivo":
    ...
    if abs(_delta_sale_sources) > 0.000001:
        ...
        st.success(
            f"Cierre correcto: {acquired_units:g} ingresadas − {sold_items_global:g} vendidas = "
            f"{stock_units:g} en stock. {money(acquired_cost)} − {money(cogs_moves_global)} = {money(stock_cost)}."
        )

    with st.expander("Ver control técnico del historial", expanded=False):
        st.write(
    ...
    # ========================================================
    # CIERRE CONTABLE SIMPLE
    # ========================================================
    st.markdown("### Cierre de mercadería")
    q1, q2, q3 = st.columns(3)
    with q1:
        pro_kpi("COMPRADAS", f"{acquired_units:g}", money(acquired_cost), "blue")
    with q2:
    ...
    if abs(_unit_balance - stock_units) < 0.000001 and abs(_cost_balance - stock_cost) < 0.01:
        ...
        st.warning(
            f"Control: unidades esperadas {_unit_balance:g} vs stock {stock_units:g} · "
            f"costo esperado {money(_cost_balance)} vs stock a costo {money(stock_cost)}."
        )
