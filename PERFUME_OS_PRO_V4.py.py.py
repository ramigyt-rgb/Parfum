...
st.markdown(
    """
    <style>
    /* Más espacio útil en tablet sin sacrificar navegación */
    section[data-testid="stSidebar"]{
    ...
    @media (max-width:1200px){
        section[data-testid="stSidebar"]{width:255px!important;min-width:255px!important;}
    ...
    try:
        ...
        for meta in FRAGRANCE_LIBRARY:
            ...
            if keeper is None:
                candidates = [
                    r for r in rows
                    if int(r.get('id') or 0) not in claimed_ids
                    and _catalog_key35(r.get('name','')) in exact_keys
                ]
                if candidates:
                    candidates.sort(
                        key=lambda r: (
